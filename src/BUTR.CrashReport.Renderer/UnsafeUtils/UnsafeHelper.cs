...
internal static unsafe class UnsafeHelper
{
    ...
    private static readonly IndexOfSpanDelegate? _indexOfSpan = null;
    /* TODO:
    private static readonly IndexOfSpanDelegate? _indexOfSpan = (IndexOfSpanDelegate) Delegate.CreateDelegate(
        typeof(IndexOfSpanDelegate),
        typeof(Span<>).Assembly.GetType("System.SpanHelpers").GetMethod("IndexOfSpan", BindingFlags.Static | BindingFlags.NonPublic, null, [typeof(byte).MakeByRefType(), typeof(byte), typeof(int)], null)!
    );
    */
